...
function generateCssVariableName(path) {
    return `--${path.join("-")}`;
}
function isObject(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
function generateMediaQuery(breakpoint, styles) {
    return `@media (min-width: ${breakpoint}) {\n    ${styles}\n  }`;
}
function processThemeValue(path, value, breakpoints) {
    const cssRules = [];
        ...
        if ("default" in value) {
            ...
            Object.entries(value).forEach(([key, val]) => {
                if (key !== "default" && breakpoints[key]) {
                    cssRules.push(generateMediaQuery(breakpoints[key], `${varName}: ${val};`));
                }
            });
        }
        else {
...
function generateThemeSection(theme, fonts) {
            ...
            Object.entries(value).forEach(([colorName, colorValue]) => {
                if (typeof colorValue === "string") {
                    ...
                    Object.keys(colorValue).forEach((shade) => {
                        colorVars.push(`--color-${colorName}-${shade}: var(--color-${colorName}-${shade});`);
                    });
                }
            });
...
function generateComponents(components, breakpoints) {
    const processComponentStyles = (baseSelector, styles) => {
        const cssRules = [];
...
function generateUtilities(utilities, breakpoints) {
    const processUtilityStyles = (selector, styles) => {
        const cssRules = [];
...
function generateBreakpointVariants(breakpoints) {
    return Object.entries(breakpoints)
        .map(([key, value]) => `@custom-variant ${key} {
...
export default function tokensToTw(tokenUrl = "assets/css/tokens.json", outputUrl = "assets/css/tokens.css") {
...
//# sourceMappingURL=tokens-to-tw.js.map